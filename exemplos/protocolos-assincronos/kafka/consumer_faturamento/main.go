...
const groupID = "faturamento-consumer-group"
...
func (h *ConsumerHandler) ConsumeClaim(session sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	for {
		...
		case message := <-claim.Messages():
			...
			// Processa a mensagem
			log.Printf("| Consumer Group: %s | Mensagem recebida | Topic: %s | Partition: %d | Offset: %d",
				groupID, message.Topic, message.Partition, message.Offset)

			log.Printf("Pedido ID: %s | Cliente: %s | Produto: %s | Quantidade: %d | Valor: R$ %.2f | Timestamp: %s",
				pedido.ID, pedido.Cliente, pedido.Produto, pedido.Quantidade, pedido.Valor, pedido.Timestamp.Format("2006-01-02 15:04:05"))
			...
			for _, header := range message.Headers {
				log.Printf("   Header: %s = %s", string(header.Key), string(header.Value))
			}
